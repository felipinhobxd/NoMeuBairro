              ...
              <div className="flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {(job.contactEmailEnabled && job.contactEmail) || (job.contactWhatsappEnabled && job.contactWhatsapp) ? (
                    <div className="mt-5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/60 p-4">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        {job.contactEmailEnabled && job.contactEmail && (
                          <div className="flex items-start gap-3 min-w-0">
                            <div className="w-9 h-9 rounded-lg bg-white dark:bg-slate-900 flex items-center justify-center shrink-0 ring-1 ring-slate-200 dark:ring-slate-700">
                              <Mail className="w-4 h-4 text-slate-600 dark:text-slate-300" />
                            </div>
                            <div className="min-w-0">
                              <p className="text-xs text-slate-500">E-mail</p>
                              <p className="font-semibold text-slate-900 dark:text-white break-all text-base">{job.contactEmail}</p>
                            </div>
                          </div>
                        )}
                        {job.contactWhatsappEnabled && job.contactWhatsapp && (
                          <a target="_blank" rel="noreferrer" href={`https://wa.me/${job.contactWhatsapp}`} className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-700 shrink-0">
