...
function createCategoryIcon(category: PostCategory) {
  ...
  return L.divIcon({ className: 'category-map-marker', html: `<div title="${cfg.label}" style="width:58px;height:58px;border-radius:18px;background:${color};border:3px solid white;box-shadow:0 5px 18px rgba(0,0,0,.3);display:flex;align-items:center;justify-content:center;font-size:28px;line-height:1;cursor:pointer;pointer-events:auto;touch-action:manipulation;user-select:none">${cfg.emoji}</div><div aria-hidden="true" style="width:12px;height:12px;background:${color};border:2px solid white;transform:rotate(45deg);position:absolute;left:23px;top:47px;box-shadow:2px 2px 4px rgba(0,0,0,.16);pointer-events:none"></div>`, iconSize: [58, 68], iconAnchor: [29, 58], popupAnchor: [0, -54] });
}

function RecenterButton({ points }: { points: [number, number][] }) {
  const map = useMap(); if (!points.length) return null;
