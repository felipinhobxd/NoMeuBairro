...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (bizRes.data) setBusinesses(bizRes.data.map((b: any) => ({
        ...
        open_time: b.open_time, close_time: b.close_time, createdBy: b.created_by, createdByName: b.users?.name || 'Morador',
        createdAt: b.created_at, avgRating: ratingsByBiz[b.id] ? ratingsByBiz[b.id].sum / ratingsByBiz[b.id].total : undefined,
      ...
      if (eventsRes.data) setEvents(eventsRes.data.map((e: any) => ({
        ...
        latitude: e.latitude, longitude: e.longitude, type: e.type, createdBy: e.created_by, createdAt: e.created_at,
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`notifications-${user.id}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'notifications', filter: `user_id=eq.${user.id}` }, () => { void fetchData(); })
      .subscribe();
  ...
  const addPost = useCallback(async (data: { title: string; description: string; category: PostCategory; location: string; imageUrl?: string; latitude?: number; longitude?: number }) => {
    ...
    if (!res.error) await fetchData(); return res;
  }, [user, fetchData]);
