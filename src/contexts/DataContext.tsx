...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (ratingsRes.data) ratingsRes.data.forEach(r => { const entry = ratingsByBiz[r.business_id] ||= { total: 0, sum: 0 }; entry.total++; entry.sum += r.stars; });

      if (user) {
        const { data: notifData } = await supabase.from('notifications').select('*, users:actor_id(name, avatar_url), posts:post_id(title), comments:comment_id(content)').eq('user_id', user.id).order('created_at', { ascending: false }).limit(20);
        if (notifData) setNotifications(notifData.map(n => ({ id: n.id, userId: n.user_id, actorId: n.actor_id, actorName: n.users?.name || 'Alguém', actorAvatarUrl: n.users?.avatar_url, type: n.type as 'support' | 'comment', postId: n.post_id, postTitle: n.posts?.title, content: n.comments?.content, isRead: n.is_read, createdAt: n.created_at })));
      }

      if (bizRes.data) setBusinesses(bizRes.data.map(b => ({ id: b.id, name: b.name, description: b.description, category: b.category, phone: b.phone, whatsapp: b.whatsapp, address: b.address, neighborhood: b.neighborhood, latitude: b.latitude, longitude: b.longitude, imageUrl: b.image_url, open_time: b.open_time, close_time: b.close_time, createdBy: b.created_by, createdByName: b.users?.name || 'Morador', createdAt: b.created_at, avgRating: ratingsByBiz[b.id] ? ratingsByBiz[b.id].sum / ratingsByBiz[b.id].total : undefined, totalRatings: ratingsByBiz[b.id]?.total || 0 })));
      if (eventsRes.data) setEvents(eventsRes.data.map(e => ({ id: e.id, title: e.title, description: e.description, date: e.event_date, location: e.location, latitude: e.latitude, longitude: e.longitude, type: e.type, createdBy: e.created_by, createdAt: e.created_at, attendanceCount: e.event_attendance?.[0]?.count ?? 0 })));
      if (commentsRes.data) setComments(commentsRes.data.map(c => ({ id: c.id, postId: c.post_id, authorId: c.author_id, authorName: c.users?.name || 'Morador', content: c.content, parentId: c.parent_id, createdAt: c.created_at })));
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('db-changes')
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'notifications' }, (payload) => { if (user && payload.new && payload.new.user_id === user.id) fetchData(); })
      .subscribe();
  ...
  const addAnonymousPost = useCallback(async (data: { tipo: string; description: string; location: string; imageUrl?: string; latitude?: number; longitude?: number }) => {
    const { data: postData, error: postErr } = await supabase.from('posts').insert({ author_id: null, category: 'seguranca', title: `Denúncia: ${data.tipo}`, description: data.description, image_url: data.imageUrl, location: data.location || 'Local Privado', latitude: data.latitude, longitude: data.longitude, is_anonymous: true }).select().single();
  ...
  const supportPost = useCallback(async (postId: string) => {
    ...
    try {
      ...
      if (existing && existing.length > 0) await supabase.from('post_supports').delete().eq('post_id', postId).eq('user_id', user.id);
      else await supabase.from('post_supports').insert({ post_id: postId, user_id: user.id });
      await fetchData();
    } finally { setProcessing(prev => { const next = new Set(prev); next.delete(postId); return next; }); }
  }, [user, processing, fetchData]);

  const addComment = useCallback(async (postId: string, content: string, parentId?: string) => { if (!user) return; await supabase.from('comments').insert({ post_id: postId, author_id: user.id, parent_id: parentId, content }); await fetchData(); }, [user, fetchData]);
  const deleteComment = useCallback(async (commentId: string) => { await supabase.from('comments').delete().eq('id', commentId); await fetchData(); }, [fetchData]);
  const deletePost = useCallback(async (postId: string) => {
    const { error } = await supabase.from('posts').delete().eq('id', postId);
    if (!error) { const ids = getMyAnonIds(); if (ids.has(postId)) { ids.delete(postId); localStorage.setItem(SK_MY_ANON, JSON.stringify([...ids])); } await fetchData(); }
  }, [getMyAnonIds, fetchData]);
  const updatePostStatus = useCallback(async (postId: string, status: PostStatus) => { await supabase.from('posts').update({ status }).eq('id', postId); await fetchData(); }, [fetchData]);
  const markNotificationsAsRead = useCallback(async () => { if (!user) return; await supabase.from('notifications').update({ is_read: true }).eq('user_id', user.id); await fetchData(); }, [user, fetchData]);
  const deleteAllNotifications = useCallback(async () => { if (!user) return; await supabase.from('notifications').delete().eq('user_id', user.id); await fetchData(); }, [user, fetchData]);
  const deleteBusiness = useCallback(async (businessId: string) => { await supabase.from('businesses').delete().eq('id', businessId); await fetchData(); }, [fetchData]);
  const deleteEvent = useCallback(async (eventId: string) => { await supabase.from('events').delete().eq('id', eventId); await fetchData(); }, [fetchData]);
  const toggleAttendance = useCallback(async (eventId: string) => {
    ...
    if (existing && existing.length > 0) await supabase.from('event_attendance').delete().eq('event_id', eventId).eq('user_id', user.id); else await supabase.from('event_attendance').insert({ event_id: eventId, user_id: user.id });
    await fetchData();
  }, [user, fetchData]);

  const getEventAttendees = useCallback(async (eventId: string) => {
    try {
      const { data, error } = await supabase.from('event_attendance').select(`id, users:user_id (name, avatar_url)`).eq('event_id', eventId);
      if (error) return [];
  ...
  const getBusinessRatings = useCallback(async (businessId: string) => {
    try {
      const { data, error } = await supabase.from('business_ratings').select(`id, stars, comment, createdAt:created_at, users:user_id (name, avatar_url)`).eq('business_id', businessId).order('created_at', { ascending: false });
      if (error) return [];
      return (data || []).map((r: any) => ({ id: r.id, stars: r.stars, comment: r.comment, createdAt: r.createdAt, userName: r.users?.name || 'Morador', userAvatarUrl: r.users?.avatar_url }));
    } catch { return []; }
  }, []);
  ...
  const reportContent = useCallback(async (data: { postId?: string; commentId?: string; reason: string }) => { await supabase.from('content_reports').insert({ reporter_id: user?.id || null, post_id: data.postId, comment_id: data.commentId, reason: data.reason }); await fetchData(); }, [user, fetchData]);
  const updateReportStatus = useCallback(async (reportId: string, status: 'resolved' | 'ignored') => { const { error } = await supabase.from('content_reports').update({ status, archived_at: new Date().toISOString(), archived_by: user?.id }).eq('id', reportId); if (error) console.error('Erro ao atualizar denúncia:', error); await fetchData(); }, [user, fetchData]);

  const isMyPost = useCallback((post: { id: string; authorId: string }) => getMyAnonIds().has(post.id) || (user ? post.authorId === user.id : false), [user, getMyAnonIds]);
